...
void LogicSystem::DealMsg() {
	for (;;) {
		...
		///找到则调用对应业务处理函数
		///如 LoginHandler / HeartBeatHandler / DealChatTextMsg 等，处理
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short &msg_id, const string &msg_data) {
	...
	//为用户设置登录 ip server（这里是服务器名称）
	std::string   ipkey = USERIPPREFIX + uid_str;
	RedisMgr::GetInstance()->Set(ipkey, server_name);

	//uid和session绑定管理,方便以后踢人操作
	UserMgr::GetInstance()->SetUserSession(uid, session);
	///以下是分布式锁（已注释，旧实现保留供参考）
	//{
	//	//此处添加分布式锁，防止多线程抢占登录
	//	//拼接用户ip对应的key
	...
	//		});
	//	//此处判断该用户是否在别处或者本处被抢先登录

	//	std::string uid_ip_value = "";
	//	auto uid_ip_key = USERIPPREFIX + uid_str;
	//	bool b_ip = RedisMgr::GetInstance()->Get(uid_ip_key, uid_ip_value);
	//	//说明用户已经登录了，此处应踢掉之前的用户登录状态
	//	if (b_ip) {
	//		//获取当前服务器ip信息
	//		auto& cfg = ConfigMgr::Inst();
	//		auto self_name = cfg["SelfServer"]["Name"];
	//		//若之前登录的服务器和当前相同，则直接在本服务器踢掉
...
void LogicSystem::AuthFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	auto touid = root["touid"].asInt(); // 被申请人的uid
